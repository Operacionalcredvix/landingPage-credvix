...
export async function initStoreLocator() {
    ...
    const { data: stores, error } = await supabase.from('lojas').select('*').order('name');
    ...
    const updateDisplayedStores = () => {
        ...
        const filteredStores = stores.filter(store => {
            ...
            const matchesSearch = store.name.toLowerCase().includes(searchTerm) || (store.address && store.address.toLowerCase().includes(searchTerm));
